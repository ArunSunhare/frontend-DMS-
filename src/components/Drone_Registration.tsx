...
//     console.log("Registration Data being sent to API:", JSON.stringify(RegistrationData, null, 2));

//     // Call the API service instead of dbManager
//     const result = await apiService.registerUser(RegistrationData);

//     if (result.success) {
//       setSubmitStatus({
//         type: 'success',
//         message: 'Registration successful! Your drone operator registration has been submitted and saved to the database. You can now login with your credentials.'
//       });
...
//                              <SelectItem value="d">OTHERS</SelectItem>
//                            </SelectContent>
//                          </Select>
//                          <FormMessage />
//                        </FormItem>
...
//    const newSpecs = [...droneSpecs];
//    newSpecs[droneIndex].droneIds.push("");
//    newSpecs[droneIndex].quantity = newSpecs[droneIndex].droneIds.length;
//    setDroneSpecs(newSpecs);
//  };
...
//   try {
//     // Update quantities based on drone IDs
//     const updatedDroneSpecs = droneSpecs.map(spec => ({ ...spec, quantity: spec.droneIds.length }));

//     // Validate drone specs with dynamic schema for camera resolution
//     for (let i = 0; i < updatedDroneSpecs.length; i++) {
//       const spec = updatedDroneSpecs[i];
//       let tempSchema = droneSpecSchema;
//       if (spec.cameraEnabled === "yes") {
//         tempSchema = droneSpecSchema.extend({
//           cameraResolution: z.string().min(1, "Camera resolution is required when camera is enabled"),
//         });
//       }
//       const result = tempSchema.safeParse(spec);
//       if (!result.success) {
//         setSubmitStatus({
//           type: 'error',
//           message: `Drone ${i + 1}: ${result.error.errors[0].message}`
...
//                                          </Button>
//                                        </div>
//                                        <p className="text-sm text-slate-500 mt-1">Quantity: {droneSpec.droneIds.length}</p>
//                                      </div>
...
import { apiService, RegistrationData } from "../services/api";
import logo from '../assets/Logo1.png';
import armyBg from '../assets/indian-army-bg.jpg.jpg'; // Add this import
...
export default function UserRegistration() {
 ...
 const handleNext = async () => {
   const isValid = await form.trigger(['abc', 'yeDiv', 'zxc', 'bde', 'corps', 'unit', 'username', 'password', 'confirmPassword']);
   if (isValid) {
     setCurrentStep(2);
   }
 };
...
const onSubmit = async (data: FormData) => {
 ...
 return (
   <div className="min-h-screen flex flex-col relative bg-cover bg-center bg-fixed" style={{ backgroundImage: `url(${armyBg})` }}>
     ...
     {/* Official Government Header */}
     <header className="relative z-10 shadow-sm bg-white/60 backdrop-blur-sm border-b-4 border-yellow-600">
       <div className="container mx-auto px-6 py-4 flex justify-between items-center">
         {/* Left - Logo + Titles */}
         <div className="flex items-center gap-4">
           <div className="w-20 h-20 bg-white rounded-full flex items-center justify-center">
     ...
     {/* Official Notice Banner */}
     <div className="relative z-10 bg-yellow-600 text-slate-800 py-2">
       <div className="container mx-auto px-4">
         <div className="flex items-center justify-center gap-2 text-sm font-medium">
           <Shield className="h-4 w-4" />
           OFFICIAL GOVERNMENT REGISTRATION PORTAL - SECURE CONNECTION
     ...
     <main className="relative z-10 container mx-auto px-4 py-8 flex-1">
       <div className="max-w-4xl mx-auto">
         {/* Official Form Header */}
         <div className="bg-white/70 backdrop-blur-sm border border-slate-300 rounded-lg p-8 mb-8 shadow-lg animate-fade-in">
           <div className="text-center mb-6">
             <div className="flex items-center justify-center gap-3 mb-4">
               <div>
                 <h1 className="text-3xl font-bold text-slate-800 tracking-wide">OFFICIAL USER REGISTRATION</h1>
           ...
           <form onSubmit={(e) => e.preventDefault()} className="space-y-8">
             ...
             <div className="flex justify-center mb-6">
               <div className="flex items-center space-x-2">
                 <div className={`w-8 h-8 rounded-full flex items-center justify-center text-sm font-bold ${currentStep === 1 ? 'bg-yellow-600 text-white' : 'bg-gray-300 text-gray-600'}`}>
                   1
                 </div>
                 <div className="w-12 h-1 bg-gray-300"></div>
                 <div className={`w-8 h-8 rounded-full flex items-center justify-center text-sm font-bold ${currentStep === 2 ? 'bg-yellow-600 text-white' : 'bg-gray-300 text-gray-600'}`}>
                   2
                 </div>
               </div>
             </div>
             ...
             {currentStep === 1 && (
               <>
                 {/* Section 1: User Details */}
                 <div className="bg-white/70 backdrop-blur-sm border border-slate-300 rounded-lg shadow-lg overflow-hidden animate-scale-in">
                   <div className="bg-slate-800 text-white px-6 py-4">
                     <div className="flex items-center gap-3">
                       <div className="bg-yellow-600 text-slate-800 rounded-full w-8 h-8 flex items-center justify-center font-bold text-sm">
                         1
                 ...
                 <div className="flex justify-end mt-6">
                   <Button
                     type="button"
                     onClick={handleNext}
                     className="bg-slate-800 hover:bg-slate-700 text-white px-12 py-4 text-lg font-bold tracking-wide uppercase shadow-lg hover:shadow-xl transition-all duration-300"
                   >
                     Next
                   </Button>
                 </div>
             ...
             {currentStep === 2 && (
               <>
                 ...
                 <div className="bg-white/70 backdrop-blur-sm border border-slate-300 rounded-lg shadow-lg p-6 animate-scale-in">
                   <div className="bg-green-50 border border-green-200 rounded-lg p-4 mb-6">
                     <div className="flex items-center gap-2 text-green-800">
                       <CheckCircle className="h-5 w-5" />
                       <p className="font-medium text-sm">
